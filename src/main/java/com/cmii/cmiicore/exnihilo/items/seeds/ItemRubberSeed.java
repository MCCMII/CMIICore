package com.cmii.cmiicore.exnihilo.items.seeds;
...
public class ItemRubberSeed extends Item implements IHasModel {
    ...
    public ItemRubberSeed() {
        ...
        for (String s : ModConfig.misc.rubberSeeds) {
    ...
    public CreativeTabs getCreativeTab() {
        return CreativeTabExNihilo.tabExNihilo;
    }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (player.canPlayerEdit(pos, facing, stack) && player.canPlayerEdit(pos.add(0, 1, 0), facing, stack)) {
            ...
            List<IPlantable> validPlants = plants.stream()
                    .filter(p -> soil.getBlock().canSustainPlant(soil, world, pos, EnumFacing.UP, p))
    ...
    public void initModel(ModelRegistryEvent e) {
        ModelLoader.setCustomModelResourceLocation(this, 0,
                new ModelResourceLocation(Reference.MOD_ID + ":item_seed", "type=rubber"));
    }
}