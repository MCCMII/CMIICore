# ExNihiloCreatio → CMIICore 方块与配套系统迁移方案

## 一、概述

将 [ExNihiloCreatio-1.12](https://github.com/BloodWorkXRaw/ExNihiloCreatio) 项目中**除筛矿体系外的方块、物品、注册表、配置、渲染与资源文件**迁移到 CMIICore 项目。

- **源项目**：`C:\Users\surface\IdeaProjects\ExNihiloCreatio-1.12`，基于 ForgeGradle 2.3，Java 8 + Kotlin 1.2
- **目标项目**：`C:\Users\surface\IdeaProjects\CMIICore`，基于 Cleanroom + Unimined，Java 25 toolchain（source/target 21），Mixin 0.8
- **策略**：逐 Phase 复制源码 → 改包名 → 排除不需要的代码 → Kotlin 转 Java → 改注册/配置方式 → 同步资源 → 每阶段编译验证
...
#### 桶模式系统（Barrel Modes）（9 个模式文件，另有 2 个桶 Handler 见上节）
...
| `CrookRegistry` | `OreRegistry` |
| `HammerRegistry`（锤子系统需要） | |
| `HeatRegistry` | |
...
| `HammerRecipe` + `HammerRecipeCategory` | 锤子 JEI |
...
| `JsonHelper` | JSON 辅助类 |
| `ItemUtil` | 物品工具（Kotlin→Java，锤子/钩子处理器和 JEI 需要） |
| `StringUtils` | 百分比格式化工具（Kotlin→Java，JEI 需要） |
...
`SieveRecipe` + `SieveRecipeCategory`
...
需要从 `.kt` 改写为 `.java` 的核心文件共 **约 41 个**（若默认配方继续复用矿辞选择逻辑，另转 `OreDictUtil.kt`）：

### 3.1 API 注册表接口（12个）

| Kotlin 文件 | 改写要点 |
|------------|---------|
| `IBarrelLiquidBlacklistRegistry.kt` | 接口方法直接转 Java |
...
| Kotlin 文件 | 说明 |
|------------|------|
| `ItemUtil.kt` | 锤子/钩子识别与 ItemStack 比较，→ Java |
| `StringUtils.kt` | JEI 百分比显示，→ Java |
| `OreDictUtil.kt` | 默认配方如继续按矿辞优先级取物品则转 Java；否则在简化默认配方时移除依赖 |
...
│   │
│   ├── api/
│   │   ├── ExNihiloCreatioAPI.java        (Kotlin→Java)
│   │   └── registries/
│   │       ├── IRegistry.java
│   │       ├── IRegistryList.java
│   │       ├── IRegistryMap.java
│   │       ├── IRegistryMappedList.java
│   │       ├── IBarrelLiquidBlacklistRegistry.java (Kotlin→Java)
...
### Phase 2：注册表接口层（~16 文件，含 12 个 KT→Java）

| 类别 | 文件 |
|------|------|
| Java 直接复制 | `IRegistry.java`, `IRegistryList.java`, `IRegistryMap.java`, `IRegistryMappedList.java` |
| KT→Java | `IBarrelLiquidBlacklistRegistry`, `ICompostRegistry`, `ICrookRegistry`, `ICrucibleRegistry`, `IFluidBlockTransformerRegistry`, `IFluidItemFluidRegistry`, `IFluidOnTopRegistry`, `IFluidTransformRegistry`, `IHammerRegistry`, `IHeatRegistry`, `IMilkEntityRegistry`, `IWitchWaterWorldRegistry` |
...
### Phase 10：JEI 兼容层（~18 文件，含 1 个 KT→Java）

| 类别 | 文件 | 改造内容 |
|------|------|---------|
| 入口 | `CompatJEI.java` | **关键**：保留 `registerHammer()`，移除 `registerSieve()` 调用和对应方法。`ExNihiloCreatio.MODID` → `Reference.MOD_ID`。`ExNihiloCreatio.configsLoaded` → 新的配置加载标记 |
...
| 锤子 | `HammerRecipe.java`, `HammerRecipeCategory.java` | 包名替换 |
...
| 排除 | `SieveRecipe`, `SieveRecipeCategory` | |
...
| 资源文件（模型JSON、材质）缺失 | 🔴 高 | 资源必须随代码迁移进入收尾 Phase；否则方块/物品会缺模型、材质和本地化 |

---

## 八、资源文件迁移（收尾 Phase，非可选）
...
这些需要复制到 `src/main/resources/assets/cmiicore/` 并排除筛矿、机械动力、矿石碎片、Pebble、EndCake 和已排除兼容模块相关资源。JSON 内的 `exnihilocreatio:` 命名空间也要改为 `cmiicore:`，否则注册名与资源定位不一致。
...
- [x] 源项目实际路径为上级目录 `C:\Users\surface\IdeaProjects\ExNihiloCreatio-1.12`
- [x] 目标项目实际路径为 `C:\Users\surface\IdeaProjects\CMIICore`
- [x] `src/main/resources/mixins.cmiicore.json` 只覆盖 `com.cmii.cmiicore.mixin.botania`，不会覆盖 `com.cmii.cmiicore.exnihilo`
- [x] 目标包 `src/main/java/com/cmii/cmiicore/exnihilo` 尚不存在，可以新建
- [ ] 迁移 Phase 0 后运行 `.\gradlew.bat compileJava --console=plain` 做基线验证（当前环境阻塞：Gradle 9.2.1 wrapper 未完整缓存且 `services.gradle.org` 超时；改用本机 Gradle 8.11.1 离线模式时又缺 Unimined 及其 classpath 依赖缓存）

Phase 1 前需确认：

- [ ] Cleanroom 是否支持 `@Config` 注解？（编译精简后的 `ModConfig` 测试）
- [ ] Cleanroom 是否支持 `RegistryEvent` 注册方式？
- [ ] CMIICore 的 `gradle/scripts/dependencies.gradle` 是否需要加入 JEI、CraftTweaker 或其他 compileOnly 依赖；本计划当前不迁移 CraftTweaker，JEI 仅在 Phase 10 前需要

---

## 十、执行记录

- 2026-05-18：完成 Phase 0 基础工具层迁移，新增 `com.cmii.cmiicore.exnihilo.util` 与 `com.cmii.cmiicore.exnihilo.json` 下的基础类。
- 2026-05-18：尝试编译验证；阻塞于 Gradle wrapper/依赖下载环境，尚未进入 Java 编译错误阶段。
